...
router.get('/', async (req,res)=>{
    try {
        ...
        const allProducts = await productsDao.getAll()// traigo todos los productos
        if(sessionName){
            console.log(`entro en sessionName  ${JSON.stringify(req.user)}`)

            const sessionCart = await cartDao.getById(sessionName.cart) //busco el carrito del usuario
            return res.render('index', {sessionName, sessionCart, allProducts}) //dibujo
        }
        console.log("No entro en sessionName")
            res.render('index', {sessionName, allProducts}) //dibujo sin el carrito
